 ...
 */
typedef void (^BDKVONotificationBlock)(id observer, id object, NSDictionary *change);


/**
 @abstract BDKVOController makes Key-Value Observing simpler and safer.
 @discussion BDKVOController adds support for handling key-value changes with blocks and custom actions, as well as the NSKeyValueObserving callback. Notification will never message a deallocated observer. Observer removal never throws exceptions, and observers are removed implicitely on controller deallocation. BDKVOController is also thread safe. When used in a concurrent environment, it protects observers from possible ressurection and avoids ensuing crash. The controller maintains a strong reference to objects observed.
 */
@interface BDKVOController : NSObject
 ...
 */
- (void)observe:(id)object keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options block:(BDKVONotificationBlock)block;
